...
def launch_args(parser):
    ...
    wf_parser.add_argument("-r", "--repository", type=str, help="GitHub repository for workflow source; DEFAULT: github.com/theiagen/public_health_bioinformatics")
    wf_parser.add_argument("-b", "--branch", type=str, help="GitHub branch for workflow source; DEFAULT: main")

    launch_parser = parser.add_argument_group("Workflow Launch Parameters")
    launch_parser.add_argument("-j", "--json", type=str, help="Path to workflow submission JSON file")
    ...
    launch_parser.add_argument("-cc", "--call_cache", action="store_true", help="Enable call caching for the workflow submission")
    launch_parser.add_argument("-c", "--comment", type=str, default="", help="User comment for the workflow submission")
    launch_parser.add_argument("-ie", "--ignore_empty", action="store_true", help="Ignore empty outputs in the workflow submission")
...
def launch(args, config=None):
    ...
    if result.ok:
        ...
        if args.preexisting:
            ...
            base_method_config_dict = generate_method_config(terra, config.repository, args.workflow, args.table, args.input_json, args.output_json, config.branch)
