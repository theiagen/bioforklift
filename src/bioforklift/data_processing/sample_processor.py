...
from .schema_models import SampleFieldAttributes, SchemaDefinition
...
class SampleDataProcessor:
    ...
    def __init__(self, schema_yaml: str):
        ...
        self.schema_definition: SchemaDefinition = convert_to_schema_definition(
            self.schema, self.field_attributes, SampleFieldAttributes
        )
    ...
    def process_samples(
        ...
        processed_df = (
            dataframe.pipe(self._apply_entity_type_mapping, config)
        ...
        logger.info(
            f"Processing complete: {len(processed_df)} samples ready for upload"
        )
        return processed_df

    def _apply_entity_type_mapping(
        self, dataframe: pd.DataFrame, config: Optional[Dict[str, Any]] = None
    ) -> pd.DataFrame:
        """
        ...
        if not sample_identifier_field:
            raise ValueError(
                "No field marked as sample_identifier in schema, sample processing requires sample_identifier marked"
            )
        ...
        if entity_column in dataframe.columns:
            ...
            dataframe = dataframe.rename(
                columns={entity_column: sample_identifier_field}
            )
        else:
    ...
    def _apply_config_field_inheritance(
        ...
        for field_name, config_field in config_fields.items():
            if config_field in config:
                ...
                logger.debug(
                    f"Inherited field '{field_name}' from config['{config_field}']"
                )
            else:
                logger.warning(
    ...
    def _map_field_names(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        for source_col, target_fields in columns_to_copy.items():
            ...
            if source_col not in schema_fields:
                ...
                logger.debug(
                    f"Keeping source column '{source_col}' (present in schema)"
                )
    ...
    def _filter_existing_samples(
        self, dataframe: pd.DataFrame, existing_identifiers: Set[str]
    ) -> pd.DataFrame:
        ...
        if not existing_identifiers:
            logger.debug(
                "No existing identifiers provided, skipping duplicate filtering"
            )
            return dataframe
        ...
        try:
            ...
            if sample_identifier_field not in dataframe.columns:
                logger.debug(
                    f"Sample identifier field '{sample_identifier_field}' not in data"
                )
                return dataframe
    ...
    def _validate_sequence_files(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if filtered_count > 0:
                logger.info(
                    f"Filtered out {filtered_count} samples without sequence files"
                )
    ...
    def _validate_field_patterns(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(
                f"Validating patterns for fields: {list(pattern_fields.keys())}"
            )
            ...
            for field_name, pattern in pattern_fields.items():
                if field_name not in dataframe.columns:
                    logger.debug(
                        f"Pattern field '{field_name}' not present in data, skipping"
                    )
                    continue
                ...
                try:
                    ...
                    if field_data.empty:
                        logger.debug(
                            f"No non-null values for pattern field '{field_name}', skipping"
                        )
                        continue
                    ...
                    if len(invalid_indices) > 0:
                        ...
                        logger.warning(
                            f"Removed {len(invalid_indices)} rows with invalid '{field_name}' values"
                        )

                except re.error as regex_error:
                    logger.error(
                        f"Invalid regex pattern for field '{field_name}': {pattern} - {regex_error}"
                    )
                    raise ValueError(
                        f"Invalid regex pattern for field '{field_name}': {regex_error}"
                    )
            ...
            if filtered_count > 0:
                ...
                if validation_failures:
                    logger.debug(
                        f"Pattern validation failures: {validation_failures[:10]}"
                    )
    ...
    def _process_date_formats(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"Processing date formats for fields: {list(date_format_fields.keys())}"
        )
        ...
        for field_name, attributes in date_format_fields.items():
            if field_name not in dataframe.columns:
                logger.debug(
                    f"Date format field '{field_name}' not present in data, skipping"
                )
                continue

            date_format = attributes.date_format
            logger.debug(
                f"Processing field '{field_name}' with date_format '{date_format}'"
            )
            ...
            if field_data.empty:
                logger.debug(
                    f"No non-null values for date field '{field_name}', skipping"
                )
                continue
            ...
            for idx, value in field_data.items():
                try:
                    ...
                    logger.debug(
                        f"Error processing date value '{value}' for field '{field_name}': {exc}"
                    )
                    invalid_indices.append(idx)
                    validation_failures.append(
            ...
            if coerced_values:
                ...
                logger.info(
                    f"Coerced {len(coerced_values)} values for field '{field_name}' to format '{date_format}'"
                )
        ...
        if filtered_count > 0:
            ...
            if validation_failures:
                logger.debug(
                    f"Date format validation failures: {validation_failures[:10]}"
                )
    ...
    def coerce_dict_types(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field_name, value in data.items():
            ...
            if field_def:
                coerced[field_name] = self._coerce_value_to_type(
                    value, field_def.field_type
                )
            else:
                logger.warning(
                    f"Field '{field_name}' not found in schema for type coercion, returning as is"
                )
    ...
    def _coerce_dataframe_types(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        for column in coerced_df.columns:
            ...
            logger.debug(
                f"Column '{column}': pandas dtype={pandas_dtype}, schema type={bq_type}"
            )

            try:
                if bq_type in ("INTEGER", "INT64"):
                    # Convert to nullable integer type
                    coerced_df[column] = pd.to_numeric(
                        coerced_df[column], errors="coerce"
                    ).astype("Int64")
                    ...
                    coerced_df[column] = (
                        coerced_df[column].map(bool_map).astype(pd.BooleanDtype())
                    )

                elif bq_type == "DATE":
                    coerced_df[column] = pd.to_datetime(
                        coerced_df[column], errors="coerce"
                    ).dt.date
                    ...
                    if bq_type == "TIMESTAMP":
                        ...
                        if coerced_df[column].dt.tz is None:
                            coerced_df[column] = coerced_df[column].dt.tz_localize(
                                "UTC"
                            )
                        else:
                ...
                logger.debug(
                    f"Converted column {column} from {pandas_dtype} to {bq_type}"
                )

            except Exception as exc:
                # Log error but continue with other columns
                logger.error(
                    f"Failed to convert column {column} to {bq_type}: {str(exc)}",
                    exc_info=True,
                )
    ...
    def get_config_identifier_field(self) -> Optional[str]:
        ...
        for field_name, attrs in self.field_attributes.items():
            if (
                attrs.get("config_identifier")
                or attrs.get("configuration_identifier")
                or attrs.get("config_id")
            ):
                return field_name
        return None
    ...
    def get_source_column_for_field(
        self, field_name: str, available_columns: List[str]
    ) -> Optional[str]:
        """
