...
class TestResolveCollectionId:
    def test_strips_whitespace_from_collection_id(self, mock_methods, make_response):
        run = RunItem.model_validate(
            {"Type": "run", "Run": {"Id": "run-1", "ExperimentName": "MyRun"}}
        )
        ...
        clauses = [
            (call.kwargs["scope"], call.kwargs["query"])
    ...
    def test_project_match_after_run_miss(self, mock_methods, make_response):
        ...
        project = ProjectItem.model_validate(
            {"Type": "project", "Project": {"Id": "proj-1", "Name": "MyProject"}}
        )
        search_mock = mock_methods.endpoints.search = MagicMock(
            side_effect=[
                make_response([], total_count=0),
    ...
    def test_match_in_both_scopes_without_priority_raises(
        self, mock_methods, make_response
    ):
        # A run and a project share the name; with no priority there is nothing to break the tie.
        run = RunItem.model_validate(
            {"Type": "run", "Run": {"Id": "run-1", "ExperimentName": "Shared"}}
        )
        project = ProjectItem.model_validate(
            {"Type": "project", "Project": {"Id": "proj-1", "Name": "Shared"}}
        )
        ...
        with pytest.raises(
            BaseSpaceCollectionIdError, match="matched in more than one scope"
        ):
            mock_methods.resolve_collection_id("Shared")

    @pytest.mark.parametrize(
        "priority, expected_id", [("runs", "run-1"), ("projects", "proj-1")]
    )
    def test_priority_picks_scope(
        self, mock_methods, make_response, priority, expected_id
    ):
    ...
    def test_ambiguous_priority_scope_raises(self, mock_methods, make_response):
        ...
        runs = [
            RunItem.model_validate(
                {
                    "Type": "run",
                    "Run": {"Id": f"run-{index}", "ExperimentName": "Shared"},
                }
            )
            for index in (1, 2)
        ]
        ...
        with pytest.raises(
            BaseSpaceCollectionIdError, match="matched 2 items in `runs`"
        ):
            mock_methods.resolve_collection_id("Shared", priority="runs")

    def test_duplicates_outside_resolved_scope_are_ignored(
        self, mock_methods, make_response
    ):
        ...
        projects = [
            ProjectItem.model_validate(
                {
                    "Type": "project",
                    "Project": {"Id": f"proj-{index}", "Name": "Shared"},
                }
            )
            for index in (1, 2)
        ]
    ...
    def test_every_scope_ambiguous_raises(self, mock_methods, make_response):
        ...
        with pytest.raises(
            BaseSpaceCollectionIdError, match="matched 3 items in `runs`"
        ):
            mock_methods.resolve_collection_id("Shared", priority="runs")
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                [
                    RunItem.model_validate(
                        {
                            "Type": "run",
                            "Run": {"Id": "run-1", "ExperimentName": "MyRun"},
                        }
                    )
                ],
                [],
            ...
            (
                ...
                [
                    RunItem.model_validate(
                        {
                            "Type": "run",
                            "Run": {"Id": "run-1", "Name": "250612_M00123_0001"},
                        }
                    )
                ],
                [],
            ...
            (
                ...
                [
                    RunItem.model_validate(
                        {
                            "Type": "run",
                            "Run": {"Id": "run-1", "ExperimentName": "MyRun2"},
                        }
                    )
                ],
                [],
            ...
            (
                "unknown",
                [OtherItem.model_validate({"Type": "sample", "Foo": "bar"})],
                [],
            ),
            ("missing", [], []),
        ],
    )
    def test_bad_match_raises(
        self, mock_methods, make_response, collection_id, run_items, project_items
    ):
        mock_methods.endpoints.search = MagicMock(
            side_effect=[
    ...
    def test_multiple_exact_matches_in_scope_raises(self, mock_methods, make_response):
        ...
        items = [
            RunItem.model_validate(
                {"Type": "run", "Run": {"Id": "run-1", "ExperimentName": "Dup"}}
            ),
            RunItem.model_validate(
                {"Type": "run", "Run": {"Id": "run-2", "ExperimentName": "Dup"}}
            ),
        ]
        mock_methods.endpoints.search = MagicMock(
...
class TestGetSearchItems:
    def test_forwards_scope_query_and_returns_items(self, mock_methods, make_response):
        items = [
            ...
            ProjectItem.model_validate(
                {"Type": "project", "Project": {"Id": "proj-1"}}
            ),
        ]
        mock_methods.endpoints.search = MagicMock(
            return_value=make_response(items, total_count=len(items))
...
class TestGetDatasets:
    def test_routes_project_scope(self, mock_methods, make_response):
        ...
        item = ProjectItem.model_validate(
            {"Type": "project", "Project": {"Id": "proj-1"}}
        )
        mock_methods.endpoints.datasets = MagicMock(
            return_value=make_response([], total_count=0)
        )
    ...
    def test_other_item_raises(self, mock_methods):
        ...
        item = OtherItem.model_validate(
            {"Type": "sample", "Sample": {"Id": "sample-1"}}
        )

        with pytest.raises(
            BaseSpaceCollectionIdError, match="Cannot list datasets for OtherItem"
        ):
            mock_methods.get_datasets(item)


class TestGetDatasetFiles:
    def test_pages_files_for_dataset(
        self, mock_methods, make_response, make_dataset, make_file
    ):
        files = [
        ...
        assert (
            mock_methods.endpoints.dataset_files.call_args.kwargs["dataset_id"]
            == "ds.1"
        )
...
class TestDownloadDatasetFileContent:
    def test_dry_run_writes_nothing(self, mock_methods, make_file, tmp_path):
        ...
        result = mock_methods.download_dataset_file_content(
            ds_file, dest_dir=tmp_path, dry_run=True
        )
    ...
    def test_streams_to_dest_dir_under_original_name(
        self, mock_methods, make_file, tmp_path
    ):
        ds_file = make_file("1", "Sample_L001_R1_001.fastq.gz", size=4)
        mock_fc = mock_methods.endpoints.file_content = MagicMock()
        mock_fc.return_value.__enter__.return_value.iter_content.return_value = [
            b"DA",
            b"TA",
        ]

        mock_methods.download_dataset_file_content(
            ds_file, dest_dir=tmp_path, progress=False
        )
...
class TestFetchSampleFastqs:
    ...
    def _wire_pipeline(
        self, mock_methods, monkeypatch, make_dataset, make_file, search_item
    ):
        ...
        monkeypatch.setattr(
            "bioforklift.basespace.basespace_methods.filter_dataset_types", mock_filter
        )
        monkeypatch.setattr(
            "bioforklift.basespace.basespace_methods.match_datasets_by_sample",
            mock_match,
        )
        monkeypatch.setattr(
            "bioforklift.basespace.basespace_methods.validate_paired_end_datasets",
            mock_validate,
        )
        monkeypatch.setattr(
            "bioforklift.basespace.basespace_methods.concatenate_dataset_files",
            mock_concat,
        )
        return {
            "ds_item": ds_item,
    ...
    def test_runs_pipeline(
        ...
        wiring = self._wire_pipeline(
            mock_methods, monkeypatch, make_dataset, make_file, search_item
        )
        ...
        assert result is None
        mock_methods.resolve_collection_id.assert_called_once_with(
            "collA", priority=None
        )
    ...
    def test_no_concatenate_skips_concat(
        self, mock_methods, tmp_path, monkeypatch, make_dataset, make_file
    ):
    ...
    def test_remove_sources_forwarded(
        self, mock_methods, tmp_path, monkeypatch, make_dataset, make_file
    ):
