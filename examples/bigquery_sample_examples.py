...
bq_samples_ops = bq.get_sample_operations(
    table_name="samples",
    sample_schema_yaml="example_sample_schema.yaml"
)

response = bq_samples_ops.load_dataframe(
    df=terra_df
)
...
updated_samples_dropped_sys_values_df = drop_system_value_columns(updated_samples_df, "example_sample_schema.yaml")
print(updated_samples_dropped_sys_values_df.head())

# # Upload data to Terra
uploaded_terra_df = terra.entities.upload_entities(data=updated_samples_dropped_sys_values_df, target="target")
