...
class TerraMerge:
    ...
    def merge_and_update_master(
        ...
        """
        logger.info(f"Starting merge of {primary_table} and {secondary_table} for update to {master_table}")
        
        # Download the primary and secondary tables , usually will be something like a sample table
        # With reads and a metadata table
        primary_df = self.terra_entities.download_table(primary_table, use_destination=use_destination)
        logger.info(f"Downloaded primary table: {primary_table} with {len(primary_df)} records")
        
        secondary_df = self.terra_entities.download_table(secondary_table, use_destination=use_destination)
        logger.info(f"Downloaded secondary table: {secondary_table} with {len(secondary_df)} records")
        
        try:
            master_df = self.terra_entities.download_table(master_table, use_destination=use_destination)
            logger.info(f"Downloaded master table: {master_table} with {len(master_df)} records")
        ...
        if not common_ids:
            logger.warning("No matching records found between primary and secondary tables")
            return pd.DataFrame()
        ...
        if primary_key != secondary_key:
            secondary_filtered = secondary_filtered.rename(columns={secondary_key: primary_key})
        
        # Determine fields to include/exclude
        all_fields = list(set(primary_filtered.columns).union(set(secondary_filtered.columns)))
        
        if fields_to_include:
            # If specific fields are included, use only those that exist
            fields_to_use = [field for field in fields_to_include if field in all_fields]
        ...
        logger.info(f"Merged result contains {len(merged_df)} rows and {len(merged_df.columns)} columns")
