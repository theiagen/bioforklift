...
class BigQueryConfigOperations:
    ...
    def create_config(self, config_data: Union[Dict[str, Any], str, Path]) -> Dict[str, Any]:
        """
    ...
    def create_configs_from_directory(self, directory_path: Union[str, Path], pattern: str = "*.json") -> List[Dict[str, Any]]:
        """
        ...
        if errors:
            print(f"Warning: {len(errors)} out of {len(json_files)} configurations failed to load")
            for error in errors:
    ...
    def update_config(self, config_id: str, update_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        ...
        for field in self.schema:
            if field.field_type.upper() == 'STRING' and field.name in processed_data:
                if isinstance(processed_data[field.name], dict) or isinstance(processed_data[field.name], list):
    ...
    def deactivate_configs(
        ...
        for i, (field, value) in enumerate(filters.items()):
            ...
            # Get field type from schema
            field_def = next((f for f in self.schema if f.name == field), None)
            param_type = parse_field_type(field_def.field_type) if field_def else "STRING"
            
            params.append(
                bigquery.ScalarQueryParameter(f"val_{i}", param_type, value)
            )
