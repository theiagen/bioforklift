...
class SentryMonitor:
    ...
    def __init__(
        ...
        if not dsn:
            raise ValueError(
                "Sentry DSN must be provided either as parameter or SENTRY_DSN environment variable"
            )
...
def init_sentry(
    ...
    **kwargs,
    ...
    return SentryMonitor(
        dsn=dsn, service_name=service_name, project_name=project_name, **kwargs
    )
