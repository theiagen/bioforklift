...
def test_init_with_custom_timeframe_validation(sample_schema_path, config_schema_path):
    ...
    with patch("bioforklift.data_processing.utils.load_schema_from_yaml") as mock_load:
        ...
        with pytest.raises(
            ValueError,
            match="Custom lookup timeframe requires lookup_days_back or lookup_hours_back",
        ):
            Terra2BQ(
                bigquery_project="test-project",
                bigquery_dataset="test-dataset",
...
""")
    ...
    mock_bigquery_instance.get_sample_operations.assert_called_once_with(
        table_name="samples", sample_schema_yaml=samples_schema
    )
    mock_bigquery_instance.get_config_operations.assert_called_once_with(
        table_name="configs", config_schema_yaml=configs_schema
    )
...
@patch("bioforklift.terra2bq.terra2bq.Terra")
def test_setup_terra_client_with_instance_values(
    mock_terra_class, sample_config, sample_schema_path, config_schema_path
):
...
def test_get_active_configs(t2bq):
    ...
    t2bq.config_ops.get_configs.assert_called_once_with(
        active_only=True, entity_type=None, skip_transferred=False
    )
...
def test_get_active_configs_with_entity_type(t2bq):
    ...
    t2bq.config_ops.get_configs.assert_called_once_with(
        active_only=True, entity_type="test_entity", skip_transferred=False
    )
...
def test_upload_to_terra_success(t2bq, sample_config, sample_df):
    ...
    t2bq.terra.entities.upload_entities.return_value = pd.DataFrame(
        {"entity_name": ["entity1", "entity2", "entity3"]}
    )
    ...
    upload_df = pd.DataFrame(
        {"entity_name": ["entity1", "entity2", "entity3"], "attr1": [1, 2, 3]}
    )
...
def test_download_with_file_transfer(
    mock_gcs_transfer_client, t2bq, sample_config, sample_df
):
...
def test_get_samples_for_submission_with_set_name(t2bq, sample_config):
    ...
    t2bq.samples_ops.get_samples_by_timeframe.return_value = pd.DataFrame(
        {"id": ["sample1", "sample2"], "entity_name": ["entity1", "entity2"]}
    )
...
def test_submit_workflow_success(t2bq, sample_config, sample_df):
    ...
    t2bq.terra.submissions.submit_workflow.return_value = {
...
def test_process_upload_and_submit_success(t2bq, sample_config):
    ...
    t2bq.sample_processor.drop_system_columns = MagicMock(
    ...
    t2bq.upload_to_terra = MagicMock(
    ...
    t2bq.submit_workflow = MagicMock(
...
def test_process_configuration_success(t2bq, sample_config):
    ...
    # Mock the steps
    t2bq.download_from_terra_to_bigquery = MagicMock(
        return_value=DownloadResult(
            status=OperationStatus.SUCCESS, loaded_count=5, filtered_count=2
        )
    )

    t2bq.process_upload_and_submit = MagicMock(
        return_value=ConfigProcessingResult(
            status=OperationStatus.SUCCESS,
            config_id=sample_config["id"],
            uploaded_count=3,
            set_name="test_set",
    ...
    result = t2bq.process_configuration(
        sample_config, None, preserve_path_structure=True
    )
...
def test_process_all_configs(t2bq):
    ...
    t2bq.process_configuration = MagicMock(
        side_effect=[
            ConfigProcessingResult(
                status=OperationStatus.SUCCESS,
                config_id="config1",
            ...
            ConfigProcessingResult(
                status=OperationStatus.NO_NEW_SAMPLES, config_id="config2"
            ),
        ]
    )
    ...
    with patch("bioforklift.terra2bq.terra2bq.sleep") as mock_sleep:
        results = t2bq.process_all_configs(
            entity_type="test_entity", batch_size=1, cooldown_seconds=0
        )
    ...
    # Check method calls
    t2bq.get_active_configs.assert_called_once_with(
        entity_type="test_entity", skip_transferred=False
    )
...
def test_sync_metadata_for_config_success(t2bq, sample_config):
    ...
    t2bq._update_bigquery_with_terra_metadata = MagicMock(
        return_value=MetadataSyncResult(
            status=OperationStatus.SUCCESS,
            bq_updated_count=3,
            updated_entities={
                "entity1": {"attr1": "val1"},
    ...
    t2bq._get_terra_data.assert_called_once_with(
        entity_type=sample_config["entity_type"], use_destination=False, page_size=None
    )
...
def test_sync_metadata_success(t2bq):
    ...
    t2bq.sync_metadata_for_config = MagicMock(
        side_effect=[
            MetadataSyncResult(
                status=OperationStatus.SUCCESS,
                config_id="config1",
...
def test_sync_metadata_for_config_calls_get_target_entity_with_use_destination_entity(
    t2bq,
):
    ...
    t2bq._get_target_entity_from_config = MagicMock(
        return_value="destination_entity_type"
    )
    ...
    t2bq._get_terra_data = MagicMock(
        return_value=DataResult(status=OperationStatus.SUCCESS, data=pd.DataFrame())
    )
    ...
    t2bq._update_bigquery_with_terra_metadata = MagicMock(
        return_value=MetadataSyncResult(
            ...
            updated_entities={
                "entity1": {"attr1": "val1"}
            },  # Non-empty to trigger destination update
            failed_updates=[],
        )
    )
...
def test_update_workflow_status_for_config_success(t2bq, sample_config):
    ...
    t2bq.samples_ops.get_unique_submission_ids.return_value = [
        "submission1",
        "submission2",
    ]
    t2bq.samples_ops.get_config_identifier_field.return_value = "config_id"
    ...
    t2bq._process_submission = MagicMock(
        side_effect=[
            SubmissionResult(
                status=OperationStatus.SUCCESS,
                workflow_count=2,
                workflow_states={"Running": 1, "Submitted": 1},
                failed_updates=[],
            ),
            SubmissionResult(
                status=OperationStatus.SUCCESS,
                workflow_count=1,
                workflow_states={"Submitted": 1},
                failed_updates=[],
            ),
        ]
    )
    ...
    t2bq._process_incomplete_workflows = MagicMock(
        return_value={
            "status": "success",
            "updated_count": 1,
            "workflow_states": {"Succeeded": 1},
            "failed_updates": [],
        }
    )

    result = t2bq.update_workflow_status_for_config(
        sample_config, days_back=7, batch_size=100, update_bigquery=True
    )
...
def test_update_workflow_status_success(t2bq):
    ...
    t2bq.update_workflow_status_for_config = MagicMock(
        side_effect=[
            WorkflowResult(
                status=OperationStatus.SUCCESS,
                config_id="config1",
...
def test_update_bigquery_with_terra_metadata_overwrite_behavior(t2bq):
    ...
    bq_samples = pd.DataFrame(
        {
            ...
            "existing_field": [
                ...
                None,
                "old_value4",
            ],  # Mix of existing, empty, and null values
    ...
    call_args = t2bq.samples_ops.bulk_update_samples.call_args[0][
        0
    ...
    sample1_update = next(
        (update for update in call_args if update["id"] == "sample1"), None
    )
    ...
    sample2_update = next(
        (update for update in call_args if update["id"] == "sample2"), None
    )
    ...
    sample3_update = next(
        (update for update in call_args if update["id"] == "sample3"), None
    )
    ...
    sample4_update = next(
        (update for update in call_args if update["id"] == "sample4"), None
    )
    ...
    for i in range(1, 5):
        sample_update = next(
            (update for update in call_args if update["id"] == f"sample{i}"), None
        )
    ...
    terra_data_identical = pd.DataFrame(
        {"entity_name": ["entity1"], "existing_field": ["same_value"]}  # Same value
    )
...
def test_update_bigquery_with_terra_metadata_allow_null_overwrite(t2bq):
    ...
    t2bq.samples_ops.bulk_update_samples = MagicMock(
        return_value={"updated_count": 1, "failed_updates": []}
    )
    ...
    t2bq.samples_ops.bulk_update_samples = MagicMock(
        return_value={"updated_count": 3, "failed_updates": []}
    )
...
def test_allow_null_overwrite_requires_overwrite_metadata(t2bq, caplog):
    ...
    t2bq.samples_ops.bulk_update_samples = MagicMock(
        return_value={"updated_count": 0, "failed_updates": []}
    )
...
def test_update_bigquery_matches_on_entity_id_column(t2bq, tmp_path):
    ...
    """
    t2bq.sample_processor = _make_processor(
        tmp_path,
        """fields:
  id:
    type: string
...
@pytest.mark.parametrize(
    "success_count,failed_updates,expected",
    [
...
def test_sync_metadata_for_config_partial_success_on_destination_failures(
    t2bq, sample_config
):
    """
