...
def config_builder(mock_schema_yaml):
    ...
    with (
        patch(
            "bioforklift.terra2bq.config_builder.BigQuery", return_value=mock_bigquery
        ),
...
class TestConfigBuilder:
    ...
    def test_init_default_location(self, mock_schema_yaml):
        ...
        with (
            patch(
                "bioforklift.terra2bq.config_builder.BigQuery"
            ) as mock_bigquery_class,
        ...
        with (
            ...
            def side_effect(file_path, *args, **kwargs):
                if str(file_path) == "test_template.json":
                    file_mock = mock_open(
                        read_data=json.dumps(mock_template)
                    ).return_value
                else:
    ...
    def test_list_terra_datatables(self, config_builder):
        ...
        config_builder.terra.entities.list_entity_types.return_value = [
            "table1",
            "table2",
        ]
        ...
        config_builder.terra.entities.list_entity_types.assert_called_once_with(
    ...
    def test_get_new_entity_types_no_pattern(self, config_builder):
        ...
        with (
            patch.object(
                config_builder,
                "list_terra_datatables",
                return_value=["table1", "table2", "table3"],
            ),
            patch.object(
                config_builder, "get_existing_entity_types", return_value=["table1"]
            ),
        ):
    ...
    def test_create_config_from_template(self, config_builder):
        ...
        config_builder.config_ops.create_config = MagicMock(
    ...
    def test_build_new_configs(self, config_builder):
        ...
        with (
            patch.object(
                config_builder, "get_new_entity_types", return_value=mock_entity_types
            ),
            patch.object(
                config_builder, "create_config_from_template", side_effect=mock_configs
            ),
        ):
    ...
    def test_build_new_configs_with_override(self, config_builder):
        ...
        with (
            ...
            patch.object(
                config_builder, "create_config_from_template", return_value=mock_config
            ) as mock_create,
        ):
            ...
            mock_create.assert_called_once_with(
                entity_type="table2", override_values=override_values
            )
