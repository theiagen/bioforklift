...
class TestTerraMerge:
    ...
    def test_merge_existing_master(self, terra_merge, mock_terra_entities):
        ...
        mock_terra_entities.download_table.assert_any_call(
            "sample", use_destination=False
        )
        mock_terra_entities.download_table.assert_any_call(
            "sample_metadata", use_destination=False
        )
        mock_terra_entities.download_table.assert_any_call(
            "sample_master", use_destination=False
        )
    ...
    def test_merge_new_master(self, terra_merge, mock_terra_entities):
        ...
        mock_terra_entities.download_table.side_effect = (
            lambda table, use_destination: (
                PRIMARY_DATA.copy()
                if table == "sample"
                else (
                    SECONDARY_DATA.copy()
                    if table == "sample_metadata"
                    else pd.DataFrame()
                )  # Empty for master table
            )
        )
        ...
        assert set(upload_data["entity:new_master_id"].tolist()) == {
            "sample1",
            "sample2",
        }
    ...
    def test_merge_with_no_matches(self, terra_merge, mock_terra_entities):
        ...
        mock_terra_entities.download_table.side_effect = (
            lambda table, use_destination: (
                pd.DataFrame({"entity:sample_id": ["sampleA", "sampleB"]})
                if table == "sample"
                else (
                    pd.DataFrame({"entity:sample_metadata_id": ["sampleC", "sampleD"]})
                    if table == "sample_metadata"
                    else pd.DataFrame()
                )
            )
    ...
    def test_merge_with_custom_keys(self, terra_merge, mock_terra_entities):
        ...
        mock_terra_entities.download_table.side_effect = (
            lambda table, use_destination: (
                pd.DataFrame({"custom_primary_id": ["sample1", "sample2"]})
                if table == "sample"
                else (
                    pd.DataFrame({"custom_secondary_id": ["sample1", "sample2"]})
                    if table == "sample_metadata"
                    else pd.DataFrame({"custom_master_id": ["sample1"]})
                )
            )
        )
    ...
    def test_merge_with_no_new_records(self, terra_merge, mock_terra_entities):
        ...
        mock_terra_entities.download_table.side_effect = (
            lambda table, use_destination: (
                pd.DataFrame({"entity:sample_id": ["sample1", "sample2"]})
                if table == "sample"
                else (
                    pd.DataFrame({"entity:sample_metadata_id": ["sample1", "sample2"]})
                    if table == "sample_metadata"
                    else pd.DataFrame(
                        {"entity:sample_master_id": ["sample1", "sample2"]}
                    )
                )
            )
        )
