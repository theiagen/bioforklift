...
class TerraSummary:
    ...
    def generate_hourly_summary(self, hours_back: int = 1, config_name_column: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        by_entity_type = []
        sample_identifier_field = self.terra2bq.samples_ops.get_sample_identifier_field()
        
        if "entity_type" in samples_df.columns:
            for entity_type, group in samples_df.groupby("entity_type"):
                sample_ids = []
                if sample_identifier_field and sample_identifier_field in samples_df.columns:
    ...
    def generate_daily_summary(self, config_name_column: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if config_id_field and config_id_field in samples_df.columns:
            for config_id, group in samples_df.groupby(config_id_field):
                ...
                by_config.append({
                    ...
                    "uploaded_samples": group["uploaded_at"].notna().sum(),
                    "submitted_samples": group["submitted_at"].notna().sum(),
                    "entity_type": config.get('entity_type', 'Unknown') if config else 'Unknown'
                })
    ...
    def generate_workflow_summary(self, days_back: int = 7) -> Dict[str, Any]:
        ...
        for config in configs:
            try:
                ...
                state_summary = self.terra2bq.samples_ops.get_workflow_state_summary(config_id)
    ...
    def format_hourly_summary_for_slack(self, summary: Dict[str, Any], project_title: str = None) -> Dict[str, Any]:
        ...
        if summary.get('by_entity_type'):
            ...
            for entity in summary['by_entity_type']:
                entity_types_text += f"• *{entity['entity_type']}*: {entity['sample_count']} samples\n"
